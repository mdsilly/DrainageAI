"""
Grayscale-compatible BYOL model for self-supervised learning with multi-view support.

This implementation extends the standard BYOL model to handle grayscale (single-channel) images.
"""
...
class GrayscaleBYOLModel(BaseModel):
    """
    BYOL model for self-supervised learning with multi-view support.
    
    This implementation supports grayscale images, RGB images, and SAR imagery.
    """
    
    def __init__(self, pretrained=True, with_sar=True, momentum=0.99):
        super(GrayscaleBYOLModel, self).__init__()
        
        # Online encoder
        self.online_encoder = models.resnet50(pretrained=pretrained)
        
        # Modify first layer to handle grayscale or SAR data
        original_conv = self.online_encoder.conv1
        
        # Create a new conv layer with flexible input channels
        if with_sar:
            # RGB + SAR (4 channels)
            in_channels = 4
        else:
            # Default to 3 channels, but we'll handle grayscale in forward pass
            in_channels = 3
        ...
        with torch.no_grad():
            # For RGB channels, copy the original weights
            new_conv.weight[:, :3] = original_conv.weight
            
            # For SAR channel (if used), initialize with small random values
            if with_sar:
    ...
    def preprocess_input(self, x):
        """
        Preprocess input to handle different channel configurations.
        
        Args:
            x: Input tensor of shape [batch_size, channels, height, width]
            
        Returns:
            Preprocessed tensor with the correct number of channels
        """
